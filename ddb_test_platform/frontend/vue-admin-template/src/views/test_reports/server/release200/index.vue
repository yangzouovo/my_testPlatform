<template>
  <div class="app-container" id="release200">
    ...
    <el-table v-loading="listLoading" :data="filtedData.slice((currentPage - 1) * pagesize, currentPage * pagesize)"
      ...
      <el-table-column label="测试类型" width="280" align="center">
        <template slot-scope="scope">
          {{ scope.row.test_type }}
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    fetchAllData() {
      ...
      getServerResults(0, null, 'release200', null).then(response => {
    ...
    //   this.listLoading = true
    //   getServerResults(1, page, 'release200',pagesize).then(response => {
